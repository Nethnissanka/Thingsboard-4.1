pipeline {
    ...
    stages {
        ...
        stage('Monitor Upgrade Progress') {
            when {
                expression { env.UPGRADE_REQUIRED == "true" }
            }
            steps {
                script {
                    echo "📊 Monitoring upgrade and startup progress..."
                    sh """
                        cd ${TB_DOCKER_PATH}
                        
                        container_name="${env.NEW_CONTAINER_NAME}"
                        timeout=900  # 15 minutes
                        elapsed=0
                        
                        echo "⏳ Waiting for upgrade and startup to complete..."
                        ...
                        while [ \$elapsed -lt \$timeout ]; do
                            ...
                            if docker logs "\$container_name" 2>&1 | grep -q "Started ThingsboardServerApplication"; then
                                echo "✅ ThingsBoard ${env.TARGET_VERSION} started successfully!"
                                break
                            fi
                            
                            # Check for upgrade completion
                            if docker logs "\$container_name" 2>&1 | grep -q "Upgrade finished successfully"; then
                                echo "✅ Database upgrade completed!"
                                sleep 10  # Wait a bit more for full startup
                                continue
                            fi
                            
                            # Check for failures
                            if docker logs "\$container_name" 2>&1 | grep -qi "error\\|exception\\|failed"; then
                                recent_logs=\$(docker logs "\$container_name" 2>&1 | tail -20)
                                if echo "\$recent_logs" | grep -qi "upgrade.*fail\\|fatal\\|error.*database"; then
                            ...
                            if [ \$((\$elapsed % 30)) -eq 0 ]; then
                                ...
                                docker logs "\$container_name" 2>&1 | tail -3 | grep -v "^$"
                            fi
                        ...
                        if [ \$elapsed -ge \$timeout ]; then
                            echo "❌ Timeout waiting for startup! Recent logs:"
                ...
                script {
                    ...
                    sh """
                        ...
                        while [ \$attempt -le \$max_attempts ]; do
                            ...
                            if timeout 15 curl -sf --max-time 10 http://localhost:8080/login > /dev/null 2>&1; then
                                echo "✅ Web interface is responding (HTTP 200)"
                                
                                # Additional check for API health
                                if timeout 10 curl -sf --max-time 5 http://localhost:8080/api/noauth/time > /dev/null 2>&1; then
                                    echo "✅ API is responding"
                                else
                                    echo "⚠️ API not responding, but login page is accessible"
                                fi
                                ...
                                echo "⏳ Web interface not ready, waiting... (\$attempt/\$max_attempts)"
